...
> Pipe"**, done fully free on GitHub: Secrets hold the credentials, Actions run
> the nightly job, Pages hosts the dashboard. Data comes from **Garmin Connect**
> (free — activities *and* recovery). Plus a **Level 3 MCP server** so you can
> ask Claude questions about your own data.
...
- Updates **automatically every night** from Garmin Connect (free).

## How it's built

| Layer | Tech |
|---|---|
| Data pipeline | Python — `python-garminconnect` for activities + wellness ([pipeline/](pipeline/)); Strava REST kept dormant |
...
| Secrets | GitHub Secrets (per-athlete Garmin token blobs) |
...
➡️ **[SETUP.md](SETUP.md)** — the secrets checklist, Garmin token minting,
